...
module.on({
	...
	exports: function(key, value) {
		if (!storage) {
			...
			modules[url] = storage = {
				...
				factory: function() {
					...
					return exports.length === 1 ? exports[0] : exports || null;
				}
			};
		...
		if ('id' in value) {
			...
			value.id = !storage.cache.length ? storage.id :storage.id + '-' + storage.cache.length;
		}
...
function loadExports(obj) {
	...
	console.log(obj.id);
	modules[obj.id] .exports = obj.exports;
	if ('id' in obj) {
		checkId(obj.id);
	}
}
...
function checkDeps(i, obj) {
	...
	each(obj.deps, function(i, id) {
		...
		if (obj.id === id || !(id in modules)) {
			...
			for (var i = 0, len = urlList.length; i < len; i += 1) {
				if (urlList[i] === id) return;
			}
			urlList.push(id);
			return;
		}
...
function checkLoaded(i, exp) {
	...
	if ('exports' in exp) {
		modules[exp.id].exports = exp.exports;
		if ('exports' in storage) {
			storage = null;
			loadModule();
		}
	}
...
function checkStorage() {
	...
	storage && storage.status === storage.cache.length && loadExports(storage);
	storage = null;
	loadModule();
}
